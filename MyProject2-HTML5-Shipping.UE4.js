...
var Module = {
	...
	UE4_indexedDBVersion: 202302011341, // Bump this number to invalidate existing IDB storages in browsers.
};
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('https://dl.dropboxusercontent.com/s/cb0315u3nlhyuve/UE4Game-HTML5-Shipping.wasm?dl=0'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/4izdiz/unreal/020fd8e03a1e7abbaba4b2a44ab4424ec305f2bf/UE4Game-HTML5-Shipping.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/4izdiz/unreal/62cb445c8025c6ec29706566c1e2063ec76ca09d/MyProject2-HTML5-Shipping.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/4izdiz/unreal/020fd8e03a1e7abbaba4b2a44ab4424ec305f2bf/Utility.js')).then(addScriptToDom);
		var dataDownload =
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://dl.dropboxusercontent.com/s/1g7kztw0vciv4o1/MyProject2-HTML5-Shipping.data?dl=0'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://dl.dropboxusercontent.com/s/1g7kztw0vciv4o1/MyProject2-HTML5-Shipping.data?dl=0')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
